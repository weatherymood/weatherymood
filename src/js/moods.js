...
var Moods = {
  ...
  codes: [
    {
      'codes': [800],
      'class': 'sun',
      'keywords': ['remission%20of%20sin', 'sunshine', 'beat']
    },
  ...
  getKeyword: (keyword) => {
    ...
    for (let i=0;i<Moods.codes.length;i++){

      if (Moods.codes[i].codes.indexOf(keyword) > -1){
        limit = Moods.codes[i].keywords.length;
        randomnumber = Math.floor(Math.random() * (limit));
        return Moods.codes[i].keywords[randomnumber];
      }
